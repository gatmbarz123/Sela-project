...
resource "aws_ssm_association" "ssm-script" {
  ...
  parameters = {
    commands = join("\n", [
      ...
      "sudo aws ecr get-login-password --region ${var.aws_region} | docker login --username AWS --password-stdin ${data.aws_caller_identity.account_id_pull.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com >> /home/ec2-user/debug.log 2>&1",
      "sudo docker pull ${data.aws_caller_identity.account_id_pull.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${var.ecr_repo_name}:v${var.image_tag} >> /home/ec2-user/debug.log 2>&1",
